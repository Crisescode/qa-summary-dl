...
class PGN(tf.keras.Model):
    ...
    def call_decoder_onestep(self, dec_input, dec_hidden, enc_output, enc_extended_inp, batch_oov_len):
        ...
        dec_x, pred, dec_hidden = self.decoder(
            dec_input, dec_hidden, enc_output, context_vector
        )

        p_gen = self.pointer(context_vector, dec_hidden, tf.squeeze(dec_x, axis=1))

        final_dists = _calc_final_dist(
            enc_extended_inp, [pred], [attention_weights], [p_gen], batch_oov_len,
    ...
    def __call__(
            self, dec_input, dec_hidden, enc_output,
        ...
        context_vector, attention_weights, coverage_ret = self.attention(
            dec_hidden, enc_output, enc_pad_mask, use_coverage, prev_coverage
        )

        for t in range(dec_target.shape[1]):
            dec_x, pred, dec_hidden = self.decoder(
                tf.expand_dim(dec_input[:, t], 1),
                dec_hidden,
                enc_output,
                context_vector
            )
            ...
            context_vector, attention_weights, coverage_ret = self.attention(
                dec_hidden, enc_output, enc_pad_mask, use_coverage, coverage_ret
            )
        ...
        final_dists = _calc_final_dist(
            enc_extended_inp, predictions, attentions, p_gens, batch_oov_len,
            self.params["vocab_size"], self.params["batch_size"]
        )
